...
export const Color = memo(function Color({
	...
	shade,
	value,
}: { name: string; shade: string; value: string }) {
	...
	function copyHexToClipboard(e: React.MouseEvent) {
		...
		const prevValue = panel.innerHTML;
		console.log("click: ", prevValue);
		if (e.shiftKey) {
			...
			<Tooltip.Root>
				<Tooltip.Trigger asChild>
					<button
						...
						className={clsx(
							"aspect-1/1 w-full  rounded-sm outline -outline-offset-1 hover:ring  outline-black/10 sm:rounded-md",
						)}
					/>
				</Tooltip.Trigger>
				<Tooltip.Portal>
					<Tooltip.Content 
						className="pointer-events-none z-10 flex translate-y-2 items-center gap-1 rounded-xl
						 border border-gray-950 bg-gray-950/90 py-0.5 pr-2 pb-1 pl-3 text-center font-mono text-xs/6 font-medium
						  whitespace-nowrap text-white opacity-100 inset-ring inset-ring-white/10 transition-[opacity] starting:opacity-0"
						sideOffset={8}
					>
						<span
							ref={(panel) => {
